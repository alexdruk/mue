...
module.exports = {
    ...
    getMarketHist: async function (exchange, symbol) {
        let ha = [];
        let toshow = 10;
        let timestamp_aday_ago = moment(new Date).subtract(1, 'days').valueOf();
        try {
            ...
            for (let i = 0; i < hist_arr.length; i++) {
                ...
                hist.time = hist_arr[i].timestamp ? moment(hist_arr[i].timestamp).format('HH:mm:ss') : '';
    ...
    parseError: function (e) {
        if (e instanceof ccxt.DDoSProtection || e.message.includes('ECONNRESET')) {
            ...
            console.log('[Exchange Not Available Error] ' + e.message + ' Try to restart later');
